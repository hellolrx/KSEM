...
# 加载保存的模型权重
model_save_path = r"D:\KSEM\ResNet\clean_model\Trojan_43k"
...
# 加载本地数据集
data_dir = r"D:\Fight-Poison-With-Poison\Trojan_poision\test\data"
...
print(f"Percentage of Class 0 Predictions: {class_zero_ratio:.2f}%")
...
#
#
# # 自定义数据集类，加载 D:\KSEM\VGG\cross_vaild_dataset\images 中的图片和标签
...
#
#         # 加载标签
#         self.labels = torch.load(label_path, weights_only=False).tolist()  # 加载为列表，假设是标量标签（如 [0, 2, 3, 7, 9]）
#
#         # 获取所有图片文件并按文件名自然排序
...
#             raise ValueError(
#                 f"Number of images ({len(self.image_files)}) does not match number of labels ({len(self.labels)})")
#
#     def __len__(self):
...
#
# # 指定路径
# image_dir = r"D:\KSEM\VGG\cross_vaild_dataset\images"
# label_path = r"D:\KSEM\VGG\cross_vaild_dataset\filtered_labels.pth"
#
# # 创建自定义数据集和数据加载器
# custom_dataset = CustomDataset(image_dir=image_dir, label_path=label_path, transform=transform)
# custom_loader = DataLoader(custom_dataset, batch_size=128, shuffle=False, num_workers=0)
#
# # 加载模型
# model = resnet.ResNet18(num_classes=10)
#
# # 加载保存的模型权重
# model_save_path = r"D:\KSEM\VGG\clean_model\blend_30K.pth"
# model.load_state_dict(torch.load(model_save_path, weights_only=True))  # 使用 weights_only=True 避免安全警告
#
# # 将模型移动到 GPU（如果可用）
# device = torch.device("cuda" if torch.cuda.is_available() else "cpu")
...
#
#     # 验证模型
#     test_loss, test_acc = validate(model, custom_loader, criterion, device)
#     print(f"Test Loss: {test_loss:.4f}, Test Acc: {test_acc:.2f}%")