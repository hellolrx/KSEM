...
def test(model, test_loader, poison_test = False, poison_transform=None, num_classes=10, source_classes=None, all_to_all = False):
    ...
    class_dist1= np.zeros((num_classes))
    ...
    with torch.no_grad():
        for data, target in test_loader:
            ...
            if poison_test:
                clean_target = target
                data, target_poison = poison_transform.transform(data, target)
                ...
                if not all_to_all:
                    ...
                    target_class = target_poison[0].item()
                    for bid in range(this_batch_size):
