...
def main():
    ...
    best_accuracy = 0.0
    save_path = r'D:\KSEM\VGG\clean_model'  # 指定保存路径

    for epoch in range(num_epochs):
        train(epoch)
        accuracy = test(testloader)  # 使用 testloader 来计算测试准确度

        if accuracy > best_accuracy:
            best_accuracy = accuracy
            torch.save(model.state_dict(), os.path.join(save_path, 'cifar10_baseline_seed0.pth'))
